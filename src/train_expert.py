...
WANDB_PROJECT = "dreamchunk-kinetix-expert"
LARGE_ENV_PARAMS = {
    "num_polygons": 12,
    "num_circles": 4,
    "num_joints": 6,
...
class NoisyActionWrapper(wrappers.UnderspecifiedEnvWrapper):
    def __init__(self, env, action_noise=0.1):
        super().__init__(env)
        self.action_noise=action_noise

    def step_env(self, key, state, action, params):
        key1, key2 = jax.random.split(key)
        action = action + jax.random.normal(key1, action.shape) * self.action_noise
...
def main(config: Config):
    ...
    env = BatchEnvWrapper(
        wrappers.LogWrapper(
            DenseRewardWrapper(
                wrappers.AutoReplayWrapper(ActionHistoryWrapper(ObsHistoryWrapper(
                    NoisyActionWrapper(env,action_noise=config.action_noise)
                    , 4)))
            )
        ),
        config.num_envs,
