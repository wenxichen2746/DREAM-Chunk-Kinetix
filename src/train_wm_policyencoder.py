...
LOG_DIR = pathlib.Path("logs-wm-policyencoder")


@dataclasses.dataclass(frozen=True)
class Config:
    run_path: str
    flow_policy_path: str
    ...
    flow_policy_step: int = -1
    wandb_name: str = "my-default-trainwm-policyencoder-name"
    model: _model.ModelConfig = dataclasses.field(
        default_factory=lambda: dataclasses.replace(_model.ModelConfig(), wm_type="policyencoder")
    )
...
class EpochCarry:
    ...
    graphdef: nnx.GraphDef[tuple[_model.FlowPolicyEncoderWorldModel, nnx.Optimizer]]
...
def _save_learning_curve(
    ...
    axes = axes.reshape(-1)
    axes[0].plot(updates_arr, train_pred_loss, marker="o", label="train")
    axes[0].plot(updates_arr, eval_pred_loss, marker="s", linestyle="--", label="eval")
    axes[0].set_ylabel("Prediction Loss")
    axes[0].grid(alpha=0.3)
    axes[0].legend(frameon=False, fontsize=8)
    axes[1].plot(updates_arr, train_loss_mean, marker="o", label="train")
    axes[1].plot(updates_arr, eval_loss_mean, marker="s", linestyle="--", label="eval")
    axes[1].set_ylabel("Total Loss")
    axes[1].grid(alpha=0.3)
    axes[1].legend(frameon=False, fontsize=8)
    axes[2].plot(updates_arr, grad_norms, marker="o")
...
def _resolve_checkpoint_dir(path: str, step: int) -> pathlib.Path:
    checkpoint_path = pathlib.Path(path)
    if (checkpoint_path / "policies").is_dir():
        return checkpoint_path
    epoch_dirs = sorted((p for p in checkpoint_path.iterdir() if p.is_dir() and p.name.isdigit()), key=lambda p: int(p.name))
    if not epoch_dirs:
        raise ValueError(f"No policy checkpoints found in {checkpoint_path}")
    return epoch_dirs[step]


def main(config: Config):
    model_config = dataclasses.replace(
        config.model,
        wm_type="policyencoder",
    ...
    if run_dir.is_dir():
        ...
        if epoch_dirs and max(int(p.name) for p in epoch_dirs) >= config.num_epochs - 1:
            print(
                f"Skipping policy-encoder WM training for {config.wandb_name}: "
    ...
    flow_checkpoint_dir = _resolve_checkpoint_dir(config.flow_policy_path, config.flow_policy_step)
    flow_state_dicts = []
    for level_path in config.level_paths:
        level_name = level_path.replace("/", "_").replace(".json", "")
        with (flow_checkpoint_dir / "policies" / f"{level_name}.pkl").open("rb") as f:
            flow_state_dicts.append(pickle.load(f))
    flow_state_dicts = jax.device_put(jax.tree.map(lambda *x: jnp.array(x), *flow_state_dicts))
    print(f"Loaded frozen FlowPolicy in_proj from {flow_checkpoint_dir}")

    @functools.partial(jax.jit, in_shardings=sharding, out_shardings=sharding)
    @jax.vmap
    def init(rng: jax.Array, flow_state_dict: dict) -> EpochCarry:
        rng, key = jax.random.split(rng)
        world_model = _model.FlowPolicyEncoderWorldModel(
            obs_dim=obs_dim,
        ...
        pure_state["in_proj"] = flow_state_dict["in_proj"]
        state.replace_by_pure_dict(pure_state)
        world_model = nnx.merge(graphdef, state)

        encoder_params = sum(x.size for x in jax.tree.leaves(nnx.state(world_model.in_proj, nnx.Param)))
        ...
        print(
            "Policy-encoder WM params: "
            f"frozen_in_proj={encoder_params:,} trainable_action_encoder={action_params:,} "
            f"trainable_predictor={predictor_params:,}"
        )
    ...
    def eval_step(epoch_carry: EpochCarry, data: generate_data.Data, eval_batch_indices: jax.Array, eval_batch_mask: jax.Array):
        ...
        def eval_minibatch(carry, minibatch):
            loss_sum, loss_sq_sum, pred_sum, count = carry
        ...
        (eval_loss_sum, eval_loss_sq_sum, eval_pred_sum, eval_count), _ = jax.lax.scan(
            eval_minibatch,
            (0.0, 0.0, 0.0, 0.0),
    ...
    epoch_carry = init(jax.random.split(rng, len(config.level_paths)), flow_state_dicts)
    history = {
