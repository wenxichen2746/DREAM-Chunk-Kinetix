...
LOG_DIR = pathlib.Path("logs-lewm")
...
class Config:
    ...
    wandb_name: str = "my-default-trainlewm-name"
    model: _model.ModelConfig = dataclasses.field(
        default_factory=lambda: dataclasses.replace(_model.ModelConfig(), wm_type="lewm")
    )
...
def _save_learning_curve(
    ...
    panels = [
        ...
        ("SIGReg Loss", train_reg_loss, eval_reg_loss),
    ]
...
def main(config: Config):
    ...
    if run_dir.is_dir():
        ...
        if epoch_dirs and max(int(p.name) for p in epoch_dirs) >= config.num_epochs - 1:
            print(
                f"Skipping LeWM training for {config.wandb_name}: "
